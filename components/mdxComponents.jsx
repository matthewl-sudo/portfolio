...
const Ptag = ({children}) => {
    return (<p className="text-400 text-md">{children}</p>);
}

const H1tag = ({id, children}) => {
    if (id) {
        return (
            <h1 className="text-3xl uppercase font-semibold mb-4 mt-4"
            id={`#${id}`} >{children}</h1>
        );
    }
    return (<h1 className="text-3xl uppercase font-semibold mb-4 mt-4">{children}</h1>)
}
...
const UlTag = ({children}) => {
    return(<ul className="p-4 rounded shadow-lg list-none text-xl text-black w-fit bg-gray-200">{children}</ul>)
}

const OlTag = ({children}) => {
    return(<ol className="p-4 rounded shadow-lg list-decimal text-xl text-black w-fit bg-gray-200">{children}</ol>)
}
...
const TableTag = ({children}) => {
    return(      
    <table className="min-w-full border-2 my-4">{children}</table>)
}

const ThTag = ({children}) => {
    return(      
        <th scope="col" className="text-md font-medium underline border-2 px-6 py-4 text-left">
        {children}
      </th>
    )
}
...
const BlockQuoteTag = ({children}) =>{
   return(
    <blockquote className="relative py-4 my-4 border-gray-300 dark:border-gray-700 border-l-8">
        <svg className="absolute top-8 left-8 transform -translate-x-6 -translate-y-8 h-8 w-8 text-gray-300 dark:text-gray-700" width="8" height="8" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
